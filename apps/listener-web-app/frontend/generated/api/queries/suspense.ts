...
export const usePodcastServiceGetApiV1PodcastCoverByProgramIdSuspense = <TData = Common.PodcastServiceGetApiV1PodcastCoverByProgramIdDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ privateKey, programId }: {
  privateKey?: string;
  programId: string;
}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UsePodcastServiceGetApiV1PodcastCoverByProgramIdKeyFn({ privateKey, programId }, queryKey), queryFn: () => PodcastService.getApiV1PodcastCoverByProgramId({ privateKey, programId }) as TData, ...options });
